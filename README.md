...
- Supplier relationships come from structured relationship extraction over 10-K customer-concentration disclosures, with per-edge provenance (accession number + verbatim quoted sentence). The live demo serves the six-company cluster it was benchmarked on — 84 edges (74 named) spanning FY2014–FY2026, with four named Apple suppliers: AVGO, CRUS, QRVO, SWKS
...
*Benchmarked against the six-company cluster (AAPL, SWKS, QRVO, CRUS, GLW, AVGO), which is also what the live demo serves — the numbers below describe the deployed system, not a wider configuration. The eval set is frozen for regression testing. Extraction has since been run over a wider set of filers locally (15 companies, 128 edges); that coverage is deliberately not deployed, so the demo and the benchmark stay the same system.*
...
flowchart TD
    ...
    FE -->|Chat tab| AG[Agent orchestration<br/>gpt-4o-mini / Claude]
...
**Storage:** Pure SQL edge table in PostgreSQL. Multi-hop traversal via `WITH RECURSIVE`. No Neo4j or Apache AGE — the deployed graph has 84 edges over six companies (128 across 15 in the wider local extraction); recursive CTE handles 2–3 hop queries trivially. All tables (financial_facts, text_chunks, supply_edges) share one Postgres instance, preserving transactional consistency and a unified audit trail.
