...
Numeric accuracy and refusal correctness have held at 100% across every run; they do not move. Retrieval went from 42.9% to 100% (7/7) via three mechanism-level fixes, not a lucky sample: (1) `retrieve_text`'s search text is no longer written by the model — a compressed keyword rewrite measurably lost the words (usually the company name) that overlapped the disclosure's actual wording, so the agent now always searches with the user's own question, verbatim; (2) the harness's passage-hit check now also credits evidence surfaced through `graph_query` (its edges carry the same disclosure sentence in `source_text`), not only `retrieve_text` — a scoring-instrument gap, not an answer-quality one; (3) `retrieve_text` gained the same ticker-typo correction (`SKWS`→`SWKS`, etc.) `query_financials`/`graph_query` already had. This is one run at default (unpinned) temperature, not a guaranteed-forever number — the Baseline and v2 rows above show this sub-metric moving 15pp between two earlier runs on the same underlying capability, and nothing here pins the sampling temperature that causes that — but every fix behind this run's number is real, committed, and independently testable, and the mechanism it targets (a compressed search query losing the words that matched) is gone regardless of what any single run's score happens to land on. Three questions have been retired since v2 — two refusal questions whose premise expired when the metrics they ask about (free cash flow, debt-to-equity inputs) were later ingested and became answerable, and one retrieval question (`ret_swks_rf_customer_concentration_risk`) whose golden phrase does not exist in the fiscal year it asks about, only in the five prior years. That third retirement is a *retroactive* correction, not just a going-forward one: every historical run that included the item — Baseline and v2 above included — scored it `correct: true`, because the harness checked only whether the phrase was retrieved, never which filing year it came from. The Baseline and v2 rows above have been recomputed to exclude it (7 retrieval items / 32 total, down from 8/33); each affected result JSON keeps both the originally-published numbers and the corrected ones side by side (`*_corrected` fields) so nothing was silently overwritten.
